...
fn pubkey_from_did_key(did_key: &str) -> PublicKey {
    ...
    assert_eq!(
        decoded[1], 0x24,
        "expected P-256 multicodec byte 1, got 0x{:02x}",
        decoded[1]
    );
    ...
    PublicKey::from_sec1_bytes(pubkey_bytes).expect("invalid P-256 public key")
}

#[test]
fn atproto_signature_verification_p256() {
    ...
    assert!(
        tested >= 3,
        "expected at least 3 P-256 fixtures, got {tested}"
    );
}

#[test]
fn w3c_didkey_p256_private_to_public() {
    let fixtures = parse(W3C_DIDKEY_P256.as_bytes()).expect("fixture JSON parse failed");
    let items = fixtures.as_array().expect("fixtures must be array");
    ...
    for fixture in items {
        ...
        let priv_bytes = decode_base58btc(priv_b58);
        assert_eq!(
            priv_bytes.len(),
            32,
        ...
        assert_eq!(
            hex_encode(&pk_sec1),
            hex_encode(&expected_sec1),
            "public key mismatch for did:key {public_did_key}"
        );
    }
}
