//! RFC 9106 に従う Argon2id 1.3 版 (PHC 版番号 19) の実装です。

use crate::blake2b::{blake2b, blake2b_long};
use neco_base64::{decode_url, encode_url};

/// Argon2id の処理パラメータです。
/// 既定値はメモリ 19456 KiB、反復 2 回、レーン 1、出力 32 バイトです。
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Argon2Params {
    /// レーン数の 8 倍以上の KiB 単位のメモリ量です。
    pub m_cost: u32,
    /// 1 以上の反復回数です。
    pub t_cost: u32,
    /// 1 以上のレーン数です。
...
/// RFC 9106 3.4 節のブロック圧縮です。`rhs XOR lhs` に行置換と列置換を適用し、元の排他的論理和を重ねます。
...
/// パスワードと salt から指定長の Argon2id ハッシュを返します。
/// 並列度と反復回数は 1 以上、メモリ量はレーン数の 8 倍以上、出力長は 4 バイト以上、salt は 8 バイト以上でなければ panic します。
...
/// secret と関連データを含めて指定長の Argon2id ハッシュを返します。
/// パラメータと salt が条件を満たさない場合は panic します。
...
/// 16 バイトの乱数 salt を用いる PHC 文字列を返します。
/// 文字列は方式、版、メモリ量、反復回数、レーン数、salt、ハッシュを表します。
...
/// PHC 文字列がパスワードと一致するときに真を返します。
/// 読み取れない文字列と一致しないパスワードには偽を返します。読み取った処理パラメータが Argon2id の制約を満たさない場合、または salt が 8 バイト未満の場合は panic します。
...
/// PHC 文字列から salt、ハッシュ、処理パラメータを読み取ります。
